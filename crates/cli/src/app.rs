...
use claude_deck_core::claude::launcher;
use claude_deck_core::config::{self, Config, SavedSession};
use claude_deck_core::cost::{self, CostInfo};
use claude_deck_core::duration::SessionDurations;
use claude_deck_core::model::session::{Session, SessionStatus};
use claude_deck_core::tmux::command::{rename_session, run_tmux_allow_failure, send_keys};
...
impl App {
    ...
    fn handle_confirm_key(&mut self, key: KeyEvent) {
        match key.code {
            KeyCode::Char('y') | KeyCode::Char('Y') => {
                if let Some(session) = self.selected_session() {
                    let name = session.name.clone();
                    match launcher::kill_session(&name) {
                        Ok(()) => {
    ...
    fn copy_preview_to_clipboard(&mut self) {
        ...
        let content: String = self
            .pane_preview
            .iter()
            .map(|line| claude_deck_core::ansi::strip_ansi(line))
            .collect::<Vec<_>>()
            .join("\n");
