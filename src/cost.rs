...
fn calculate_cost(info: &CostInfo, pricing: &ModelPricing) -> f64 {
...
fn find_latest_session_file(project_dir: &str) -> Option<PathBuf> {
...
fn parse_jsonl_file(path: &std::path::Path) -> CostInfo {
...
fn extract_usage(line: &str) -> Option<UsageTokens> {
